...
function drawWave() {
  ...
  if (r > 70) {
    ...
    if (r > 30) {
      flys1.push(new Fly(random()*10+map(pose.rightEye.x,0,CAM_WIDTH,width ,0), map(pose.rightEye.y,0,CAM_HEIGHT,0 ,height) + random()+10));
    }
  }

  // if (mouseIsInside) {
    // flys2.push(new Fly(random() * 10 + 271, 244 + random() * 20));
  // }
