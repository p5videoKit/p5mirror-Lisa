...
//   for(let i=0;i<7;i++){
//     circle(Deyelashes[i][1].pos.x,Deyelashes[i][1].pos.y,10)
//     circle(Deyelashes[i][0].pos.x,Deyelashes[i][0].pos.y,10)
//   }
 }
...
//   for(let i=0;i<7;i++){
//     circle(eyelashes[i][1].pos.x,eyelashes[i][1].pos.y,10)
//     circle(eyelashes[i][0].pos.x,eyelashes[i][0].pos.y,10)
  //}
}
...
function Dpartsetup(){
  for (let i = 0; i < 7; i++) {
    for (let a = 0; a < 8; a++) { //5
      Deyelashes[i].push(new Ball(10*a, 0, 5));
    }
  }
  ...
  for (let i = 0; i < Deyelashes.length; i++) {
    ...
    for (let a = 0; a < Deyelashes[i].length - 1; a++) {
      DlinkEyes[i].push(new Spring(Deyelashes[i][a], Deyelashes[i][a + 1], 2, 1));
    }
  }
}
...
function behavior(){
 ...
 //console.log(eat,yScale,scl,n)
  if(Back){
    count++
    if(scl!=1){
      scl=scl-0.01
      yScale=map(noise(frameCount*0.05),0,1,-0.8,-0.1)
    } if(scl==1){
      if(count>450){
        n=0
        count=0
        ...
        yScale=map(noise(frameCount*0.04),0,1,-0.8,-0.1)
      }
    }
  }
}