...
function displayBubbles() {
  bubbles.forEach((bubble) => {
    ...
    if (segment) {
      ...
      textSize(14);
      let bubbleWidth = max(textWidth(bubble.text) + 20, 120); // Dynamic bubble width based on text length
      let bubbleHeight = 60;

      fill(255);
      stroke(0);
      strokeWeight(1);
      rect(bubbleX, bubbleY, bubbleWidth, bubbleHeight, 10); // Draw bubble rectangle

      fill(0);
      noStroke();
      text(bubble.text, bubbleX + 10, bubbleY + 20, bubbleWidth - 20, bubbleHeight - 20); // Text inside the bubble
    }
  });
}
