...
let ruleSets = {
  "set1": [{ a: "F", b: "F-F[-F-F]+[-F]" },
         {a:"X", b:"F-[+FF]+[-FF]"}],
 "set2": [{ a: "F", b: "FF-[-F+F]+[+F-F-F]" },
         {a:"X", b:"X"}],
  "set3": [{ a: "F", b: "FF" },
         {a:"X", b:"F-[X+X]+F-F[C3+FX]F-FFX"}],
  "set4": [{ a: "F", b: "F[+F--F][-F++-F]--F++F" },
         {a:"X", b:"X"}]
};
...
class LSystem {
  ...
  update() {
    ...
    this.rules=ruleSets[ui.ruleSet]
  ...
  draw() {
    ...
    for (let i = 0; i < this.sentence.length; i++) {
      ...
      if (current == "F") {
        ...
        ellipse(0, 0, this.len/ui.rad, this.len/ui.rad );
        rotate(shake);
      } else if (current == "+") {
        rotate(angle + shake);
      } else if (current == "-") {
...
function setup() {
  ...
  gui.add(ui, "wholeAngle", -50, 60);
  gui.add(ui, "generation", 2, 5).onChange(update).step(1);
  gui.add(ui, "frequency", 0.001, 0.01);
  gui.add(ui, "rad", 0, 10);
  //gui.add(ui,"ruleSet",Object.keys(ruleSets)).onChange(update)

  let dropdown=gui.add(ruleSets,"set1",Object.keys(ruleSets))
  dropdown.onChange(update)
  lSystem = new LSystem(axiom, ruleSets[ui.ruleSet]);
  lSystem.update();
}
