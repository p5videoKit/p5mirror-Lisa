...
class Creature2 {
  constructor(x = 0, y = 0, angle = 0) {
    ...
    for (let i = 0; i < 8; i++) {
    this.balls.push(new Ball(i, i, 30));
    stroke(0);
  }
  ...
  for (let i = 0; i < this.balls.length; i++) {
    ...
    if (i != this.balls.length - 1) {
      ...
      this.springs.push(
        new Spring(this.balls[i], this.balls[0], 1.6 * 40 * sqrt(2 - 2 / sqrt(2)), 0.9)
      );
    }
  }
  for (let i = 0; i < this.balls.length / 2; i++) {
    this.springs.push(new Spring(this.balls[i], this.balls[i + this.balls.length / 2], 580, 0.12));
  }
  }
  ...
  fill(248, 224, 15);
  circle(
    this.balls[7].pos.x + (this.balls[1].pos.x - this.balls[7].pos.x) / 2,
    this.balls[1].pos.y + (this.balls[7].pos.y - this.balls[1].pos.y) / 2,
    50
  );
  circle(
    this.balls[7].pos.x + (this.balls[3].pos.x - this.balls[7].pos.x) / (1 + 1.8 * sqrt(2)),
    this.balls[3].pos.y + (this.balls[7].pos.y - this.balls[3].pos.y) / (1.2 * sqrt(2)),
    50
  );
  ...
  update() {
    ...
    rotate(this.angle);
    scale(0.1);   
    this.balls[0].pos.x+=mSin(frameCount*0.12)*60
    this.drawStructure()
    pop();
  }
}
