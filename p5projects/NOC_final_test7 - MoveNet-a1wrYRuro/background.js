...
function drawTinyFish() {
  ...
  for (let i = 0; i < floor(ui.Num*2.2); i++) {
    let v = vehicles[i];
    ...
    creatures2[i].x=vehicles[i].pos.x
    creatures2[i].y=vehicles[i].pos.y
    creatures2[i].angle=vehicles[i].vel.heading()
...
class Fly {
  ...
  display() {
    ...
    push();
    strokeWeight(4);
    rotate(map(mSin(frameCount * 0.9), -1, 1, -PI / 6 - 0.5, PI / 6 + 0.5));
    line(0, 0, 15, 0);
    pop();
    push();
    strokeWeight(2);
    rotate(
      map(mSin(frameCount * 0.9), -1, 1, (7 * PI) / 6 + 0.5, (5 * PI) / 6 - 0.5)
    );
    line(0, 0, 15, 0);
    pop();
    pop();
