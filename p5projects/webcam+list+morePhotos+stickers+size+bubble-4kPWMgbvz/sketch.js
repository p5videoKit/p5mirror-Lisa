...
let video;
let frame; // Store the captured frame(more than one;-D)

let captureBtn, nextStepBtn, enlargeBtn, shrinkBtn, createBubbleBtn;
let state = "camera"; // state:'camera', 'selection', 'display', 'sticker'
let showCamera = true;
...
function setup() {
  ...
  createBubbleBtn.position(400, 500);
...
function createBubble() {
  if (selectedSticker) {
    let bubble = {
      ...
      dialogue: "", // Start with empty dialogue
      dragging: false, 
      input: createInput() 
    };

    bubble.input.position(bubble.x + 10, bubble.y + 10);//inside the bubble
...
// Resize the currently selected sticker变大变小真的奇妙；）
...
function resetCamera() {
  ...
  bubbles = []; // Clear bubbles on reset
