...
function sendMessage() {
  ...
  messages = [{
    role: "user",
    content:'I would like you to generate a cute face expression with pixel based on the following keywords. In your response, only tell me in this format：“{"background": "a hex color value","circles": [{"color": "a hex color value","size": a certain number, "x": a certain number,"y": a certain number},{"color": "a hex color value","size": a certain number,"x": a certain number,"y": a certain number}]}”.You should create more than one circle to make eyes face nose, head, mouse, and accessry like tears or cheek.For each eye, use at least two circles with different colors. Make sure it is valid JSON and the cute face and head is fully shown on the canvas 400*400. You can use arc or circle as mouse as you want.Here are the keywords:' + select("#content").value(),
  }];
