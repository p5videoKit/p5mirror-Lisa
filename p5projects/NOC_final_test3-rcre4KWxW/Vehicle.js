class Vehicle {
  ...
  update() {
    ...
    this.vel.limit(1.5)
    this.pos.sub(createVector(-1.8,0))
...
class Flock {
  ...
  generate() {
    for (let i = 0; i < Num; i++) {
      let x =(i+1)*width/(Num+1)+random()*60;
      let y = height/2+random(-1,1)*100;
...
class Boid {
  ...
  flock(others) {
    let noiseVec = p5.Vector.fromAngle(noise(this.pos.x * 0.005, this.pos.y * 0.005) * TWO_PI).limit(this.maxSteerForce);
    this.applyForce(noiseVec);
    
    let mousePos = createVector(mouseX, mouseY);
    let centerPos=createVector(this.pos.x-10,height/2-20)
