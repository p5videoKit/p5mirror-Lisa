...
drawTinyFish()
  ...
  if (mouseIsInside) {
    ...
    transition += (1 - transition) * 0.015;
  } else {
    if (mountainColor > 0) {
      mountainColor -= 2.5;
    }
    transition -= transition * 0.015;
  }
...
function drawWave() {
  ...
  if (mouseIsInside) {
    flys2.push(new Fly(random() * 40 + mouseX, mouseY + random() * 40));
  }
...
class Fly {
  ...
  display() {
    ...
    strokeWeight(2);
    rotate(
      map(sin(frameCount * 0.8), -1, 1, (7 * PI) / 6 + 0.5, (5 * PI) / 6 - 0.5)
    );
    line(0, 0, 15, 0);
    pop();
    pop();
